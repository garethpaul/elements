...
var box = require('@twilio-paste/box');
...
var flex = require('@twilio-paste/flex');
var text = require('@twilio-paste/text');
...
const createResolvedObject = (currentObject, options = {}) =>
...
const recursivelyCreateResolvedObject = (
  ...
  const resolvedObjectProxy = new Proxy(currentObject, {
    get(target, name) {
      ...
      if (lodash.isPlainObject(resolvedValue) || lodash.isArray(resolvedValue)) {
...
const useResolvedObject = currentObject => {
  ...
  return React__namespace.useMemo(
    () => createResolvedObject(currentObject, { contextObject: document, resolver }),
...
function ElementsOptionsProvider({ children, nodeHasChanged }) {
  return React__namespace.createElement(
    ...
    { value: Object.assign({}, DEFAULT_CONTEXT, { nodeHasChanged }) },
    children,
  );
}

function isSMDASTRoot(maybeAst) {
  return lodash.isObject(maybeAst) && maybeAst['type'] === 'root' && mosaic.isArray(maybeAst['children']);
}
function isJSONSchema(maybeSchema) {
  return lodash.isPlainObject(maybeSchema);
}
function isStoplightNode(maybeNode) {
  return lodash.isObject(maybeNode) && 'id' in maybeNode;
}
...
function useParsedData(nodeType, data) {
  return React__namespace.useMemo(() => {
    var _a;
    return (_a = parserMap[nodeType]) === null || _a === void 0 ? void 0 : _a.call(parserMap, data);
  }, [nodeType, data]);
}
const parserMap = {
...
const ArticleComponent = React__namespace.memo(({ data }) => {
  return React__namespace.createElement(
    mosaic.Box,
    { className: 'sl-elements-article' },
    React__namespace.createElement(MarkdownViewer, {
      className: 'sl-elements-article-content',
      markdown: data,
      includeToc: true,
    }),
  );
...
const NodeTypeIconDefs = {
  ...
  image: freeSolidSvgIcons.faImage,
};
...
const getServersToDisplay = (originalServers, mockUrl) => {
  const servers = originalServers
    .map((server, i) => {
      ...
      return Object.assign(Object.assign({}, server), {
...
const RequestSamples = React__default['default'].memo(({ request, embeddedInMd = false }) => {
  ...
  return React__default['default'].createElement(
    mosaic.Panel,
    { rounded: embeddedInMd ? undefined : true, isCollapsible: embeddedInMd },
    React__default['default'].createElement(
      mosaic.Panel.Titlebar,
      {
        rightComponent: React__default['default'].createElement(mosaic.CopyButton, {
          size: 'sm',
          copyValue: requestSample || '',
        }),
      },
      React__default['default'].createElement(
        mosaic.Box,
        { ml: -2 },
        React__default['default'].createElement(mosaic.Menu, {
          ...
          renderTrigger: ({ isOpen }) =>
            React__default['default'].createElement(
              mosaic.Button,
              { size: 'sm', iconRight: 'chevron-down', appearance: 'minimal', active: isOpen },
              'Request Sample: ',
              selectedLanguage,
              ' ',
    ...
    React__default['default'].createElement(
      mosaic.Panel.Content,
      { p: 0 },
      React__default['default'].createElement(mosaicCodeViewer.CodeViewer, {
        'aria-label': requestSample !== null && requestSample !== void 0 ? requestSample : fallbackText,
        noCopyButton: true,
        maxHeight: '400px',
...
const isOAuth2ImplicitFlow = maybeFlow =>
  lodash.isObject(maybeFlow) && 'authorizationUrl' in maybeFlow && !('tokenUrl' in maybeFlow);
...
const APIKeyAuth = ({ scheme, onChange, value }) => {
  return React__namespace.createElement(
    mosaic.Panel.Content,
    { className: 'ParameterGrid' },
    React__namespace.createElement(AuthTokenInput, {
      type: 'apiKey',
      name: scheme.name,
      value: value,
      onChange: onChange,
...
const DigestAuth = ({ onChange, value }) => {
  return React__namespace.createElement(
    ...
    React__namespace.createElement('textarea', {
      className:
        'sl-relative sl-z-10 sl-w-full sl-text-base sl-bg-canvas-100 sl-p-1 sl-pr-2.5 sl-pl-2.5 sl-rounded sl-border-transparent hover:sl-border-input focus:sl-border-primary sl-border',
...
const TryItAuth = ({ operationSecurityScheme: operationAuth, onChange, value }) => {
  var _a;
  const operationSecurityArray = lodash.flatten(operationAuth);
  const filteredSecurityItems = operationSecurityArray.filter(scheme =>
    securitySchemeKeys.includes(scheme === null || scheme === void 0 ? void 0 : scheme.type),
  );
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Panel.Titlebar,
      {
        rightComponent:
          ...
          React__namespace.createElement(mosaic.Menu, {
            ...
            renderTrigger: ({ isOpen }) =>
              React__namespace.createElement(
                mosaic.Button,
                { appearance: 'minimal', size: 'sm', iconRight: ['fas', 'sort'], active: isOpen },
                menuName,
              ),
          }),
      },
    ...
    React__namespace.createElement(SecuritySchemeComponent, {
      scheme: value ? value.scheme : filteredSecurityItems[0],
      onChange: handleChange,
      value: (_a = value && value.authValue) !== null && _a !== void 0 ? _a : '',
    }),
  );
};
const GenericMessageContainer = ({ scheme }) => {
  return React__namespace.createElement(mosaic.Panel.Content, null, 'Coming Soon: ', getReadableSecurityName(scheme));
};
const SecuritySchemeComponent = _a => {
  var { scheme } = _a,
    rest = tslib.__rest(_a, ['scheme']);
  switch (scheme.type) {
    case 'apiKey':
...
const caseInsensitivelyEquals = lodash.curry((a, b) => a.toUpperCase() === b.toUpperCase());
...
const isApiKeySecurityScheme = maybeIApiKey => lodash.isObject(maybeIApiKey) && maybeIApiKey.type === 'apiKey';
const isOAuth2SecurityScheme = maybeIOAuth2 => lodash.isObject(maybeIOAuth2) && maybeIOAuth2.type === 'oauth2';
const isBasicSecurityScheme = maybeIBasic =>
  lodash.isObject(maybeIBasic) && maybeIBasic.type === 'http' && maybeIBasic.scheme === 'basic';
const isBearerSecurityScheme = maybeIBearer =>
  lodash.isObject(maybeIBearer) && maybeIBearer.type === 'http' && maybeIBearer.scheme === 'bearer';
const isDigestSecurityScheme = maybeIBearer =>
  lodash.isObject(maybeIBearer) && maybeIBearer.type === 'http' && maybeIBearer.scheme === 'digest';
function filterOutAuthorizationParams(queryParams, securitySchemes = []) {
  const flattenedSecuritySchemes = lodash.flatten(securitySchemes);
...
const getSecuritySchemeNames = securitySchemes =>
...
const FileUploadParameterEditor = ({ parameter, value, onChange }) => {
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      'label',
      { 'aria-hidden': 'true', 'data-testid': 'param-label', htmlFor: parameterInputId },
    ...
    React__namespace.createElement(
      ...
      React__namespace.createElement(mosaic.Input, {
        ...
        type: 'text',
        required: true,
        value: (_a = value === null || value === void 0 ? void 0 : value.name) !== null && _a !== void 0 ? _a : '',
        disabled: true,
      }),
      value &&
        React__namespace.createElement(
          'button',
          { className: 'sl-mr-3 sl-p-2', 'aria-label': 'Remove file', onClick: clearFile },
...
function enumOptions(enumValues, required) {
  const options = lodash.map(enumValues, v => ({ value: typeof v === 'number' ? v : String(v) }));
...
function getPlaceholderForParameter(parameter) {
  ...
  return String(
    (_b = (_a = parameter.schema) === null || _a === void 0 ? void 0 : _a.type) !== null && _b !== void 0 ? _b : '',
  );
}
function retrieveDefaultFromSchema(parameter) {
  var _a;
  const defaultValue = (_a = parameter.schema) === null || _a === void 0 ? void 0 : _a.default;
  return lodash.isObject(defaultValue) ? json.safeStringify(defaultValue) : defaultValue;
}
const getValueForParameter = parameter => {
  ...
  const enums =
    (_c = (_b = parameter.schema) === null || _b === void 0 ? void 0 : _b.enum) !== null && _c !== void 0 ? _c : [];
  if (enums.length > 0) {
...
const initialParameterValues = params => {
  const paramsByName = lodash.keyBy(params, param => param.name);
  return lodash.mapValues(paramsByName, param => getInitialValueForParameter(param));
};
    ...
    Object.assign(
      {
        ...
        examples:
          typeof schema !== 'boolean' && schema.examples ? [{ key: 'example', value: schema.examples }] : undefined,
      },
      (required === null || required === void 0 ? void 0 : required.includes(name)) && { required: true },
    ),
  );
}
...
const ParameterEditor = ({ parameter, value, onChange, isOptional, onChangeOptional, canChangeOptional, validate }) => {
  ...
  const selectedExample =
    (_a = examples === null || examples === void 0 ? void 0 : examples.find(e => e.value === value)) !== null &&
    _a !== void 0
      ? _a
      : selectExampleOption;
  const parameterDisplayName = `${parameter.name}${parameter.required ? '*' : ''}`;
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Text,
      { as: 'label', 'aria-hidden': 'true', 'data-testid': 'param-label', htmlFor: inputId, fontSize: 'base' },
      parameterDisplayName,
    ),
    React__namespace.createElement(mosaic.Text, { mx: 3 }, ':'),
    React__namespace.createElement(
      ...
      parameterValueOptions
        ? React__namespace.createElement(mosaic.Select, {
            flex: 1,
            'aria-label': parameter.name,
        ...
        : React__namespace.createElement(
            ...
            React__namespace.createElement(mosaic.Input, {
              ...
              type:
                ((_b = parameter.schema) === null || _b === void 0 ? void 0 : _b.type) === 'number' ? 'number' : 'text',
              required: true,
...
const FormDataBody = ({ specification, values, onChangeValues, onChangeParameterAllow, isAllowedEmptyValues }) => {
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Panel.Content,
      { className: 'sl-overflow-y-auto ParameterGrid OperationParametersContent' },
      mapSchemaPropertiesToParameters(parameters, required).map(parameter => {
        ...
        if (supportsFileUpload) {
          return React__namespace.createElement(FileUploadParameterEditor, {
            key: parameter.name,
            parameter: parameter,
            ...
            onChange: newValue =>
              newValue
                ...
                : onChangeValues(lodash.omit(values, parameter.name)),
          });
        }
        return React__namespace.createElement(ParameterEditor, {
          key: parameter.name,
          parameter: parameter,
          ...
          onChangeOptional: value =>
            onChangeParameterAllow(Object.assign(Object.assign({}, isAllowedEmptyValues), { [parameter.name]: value })),
          canChangeOptional: true,
          isOptional: (_a = isAllowedEmptyValues[parameter.name]) !== null && _a !== void 0 ? _a : false,
        });
      }),
    ),
  );
...
function createRequestBody(mediaTypeContent, bodyParameterValues) {
  ...
  return tslib.__awaiter(this, void 0, void 0, function* () {
    ...
    const creator =
      (_a = yield requestBodyCreators[mediaTypeContent.mediaType.toLowerCase()]) !== null && _a !== void 0
        ? _a
        : createRawRequestBody;
    return creator({ mediaTypeContent, bodyParameterValues, rawBodyValue: '' });
  });
}
const createUrlEncodedRequestBody = ({ bodyParameterValues = {} }) =>
  tslib.__awaiter(void 0, void 0, void 0, function* () {
    const filteredValues = lodash.pickBy(bodyParameterValues, lodash.isString);
    return new URLSearchParams(filteredValues);
  });
const createMultipartRequestBody = ({ mediaTypeContent, bodyParameterValues = {} }) =>
  tslib.__awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    const formData = new FormData();
    for (const [key, value] of Object.entries(bodyParameterValues)) {
      const schema =
        (_b = (_a = mediaTypeContent.schema) === null || _a === void 0 ? void 0 : _a.properties) === null ||
        _b === void 0
          ? void 0
          : _b[key];
...
const createRawRequestBody = ({ rawBodyValue = '' }) =>
  tslib.__awaiter(void 0, void 0, void 0, function* () {
    return rawBodyValue;
  });
...
const useBodyParameterState = mediaTypeContent => {
  ...
  const initialState = React__namespace.useMemo(() => {
    ...
    const properties =
      (_b =
        (_a = mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.schema) === null ||
        _a === void 0
          ? void 0
        ...
        : {};
    const required =
      (_c = mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.schema) === null ||
      _c === void 0
        ? void 0
        : _c.required;
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Panel.Titlebar,
      {
        rightComponent:
          examples.length > 1 &&
          React__namespace.createElement(ExampleMenu, {
            examples: examples,
    ...
    React__namespace.createElement(
      mosaic.Panel.Content,
      { className: 'TextRequestBody' },
      React__namespace.createElement(mosaicCodeEditor.CodeEditor, {
        onChange: onChange,
        language: 'json',
        value: requestBody,
...
function ExampleMenu({ examples, requestBody, onChange }) {
  const handleClick = React__namespace.useCallback(
    example => {
      var _a;
      onChange(
        (_a = json.safeStringify('value' in example ? example.value : example.externalValue, undefined, 2)) !== null &&
          _a !== void 0
          ? _a
          : requestBody,
      );
...
const useGenerateExampleFromMediaTypeContent = (
  mediaTypeContent,
  chosenExampleIndex,
  { skipReadOnly, skipWriteOnly, skipNonRequired, ticks } = {},
) => {
  const document = useDocument();
...
const generateExampleFromMediaTypeContent = (mediaTypeContent, document, chosenExampleIndex = 0, options) => {
  ...
  const textRequestBodySchema =
    mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.schema;
  const textRequestBodyExamples =
    mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.examples;
  try {
    if (
      textRequestBodyExamples === null || textRequestBodyExamples === void 0 ? void 0 : textRequestBodyExamples.length
    ) {
      return (_a = json.safeStringify(
        textRequestBodyExamples === null || textRequestBodyExamples === void 0
          ? void 0
          : textRequestBodyExamples[chosenExampleIndex]['value'],
        undefined,
        2,
      )) !== null && _a !== void 0
        ? _a
      ...
      return generated !== null
        ? (_b = json.safeStringify(generated, undefined, 2)) !== null && _b !== void 0
          ? _b
          : ''
        : '';
    }
...
const getServerUrl = ({ chosenServer, httpOperation, mockData, corsProxy }) => {
  ...
  const serverUrl =
    (mockData === null || mockData === void 0 ? void 0 : mockData.url) || chosenServerUrl || window.location.origin;
...
const getQueryParams = ({ httpOperation, parameterValues }) => {
  ...
  return query.reduce((acc, param) => {
    ...
    if (
      ((_c = param.schema) === null || _c === void 0 ? void 0 : _c.type) === 'object' &&
      param.style === 'form' &&
      value
    ) {
      let nested;
      ...
      if (explode) {
        ...
        acc.push({
          name: param.name,
          value: nested.join(
            (_e = delimiter[param.style]) !== null && _e !== void 0 ? _e : delimiter[types.HttpParamStyles.Form],
          ),
        });
      }
    } else {
...
function buildFetchRequest({
  ...
  return tslib.__awaiter(this, void 0, void 0, function* () {
    ...
    const rawHeaders = filterOutAuthorizationParams(
      (_b = (_a = httpOperation.request) === null || _a === void 0 ? void 0 : _a.headers) !== null && _b !== void 0
        ? _b
        : [],
      ...
      .map(header => {
        var _a;
        return { name: header.name, value: (_a = parameterValues[header.name]) !== null && _a !== void 0 ? _a : '' };
      })
    ...
    const headers = Object.assign(
      Object.assign(
        Object.assign(
          {},
          (mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.mediaType) !==
            'multipart/form-data' && {
            'Content-Type':
              (_c = mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.mediaType) !==
                null && _c !== void 0
                ? _c
                : 'application/json',
          },
      ...
      mockData === null || mockData === void 0 ? void 0 : mockData.header,
    );
    return [
      urlObject.href,
      {
...
const runAuthRequestEhancements = (auth, queryParams, headers) => {
  ...
  if (isDigestSecurityScheme(auth.scheme)) {
    newHeaders.push({
      name: 'Authorization',
      value:
        (_e = (_d = auth.authValue) === null || _d === void 0 ? void 0 : _d.replace(/\s\s+/g, ' ').trim()) !== null &&
        _e !== void 0
          ? _e
          : '',
    });
...
function buildHarRequest({
  ...
  return tslib.__awaiter(this, void 0, void 0, function* () {
    const serverUrl = getServerUrl({ httpOperation, mockData, chosenServer, corsProxy });
    const mimeType =
      (_a = mediaTypeContent === null || mediaTypeContent === void 0 ? void 0 : mediaTypeContent.mediaType) !== null &&
      _a !== void 0
        ? _a
        : 'application/json';
    const shouldIncludeBody = ['PUT', 'POST', 'PATCH'].includes(httpOperation.method.toUpperCase());
    ...
    const headerParams =
      (_d =
        (_c = (_b = httpOperation.request) === null || _b === void 0 ? void 0 : _b.headers) === null || _c === void 0
          ? void 0
          : _c.map(header => {
              var _a;
              return {
                name: header.name,
                value: (_a = parameterValues[header.name]) !== null && _a !== void 0 ? _a : '',
              };
            })) !== null && _d !== void 0
        ? _d
        : [];
...
function supportsResponseCode(httpOperation, code) {
  var _a;
  return (
    ((_a = httpOperation.responses) === null || _a === void 0
      ? void 0
      : _a.find(response => response.code === code)) !== undefined
  );
}
function supportsExample(httpOperation, code, exampleKey) {
  ...
  const response =
    (_a = httpOperation.responses) === null || _a === void 0 ? void 0 : _a.find(response => response.code === code);
  if (!response) return false;
  const exampleKeys = lodash.uniq(
    (_b = response.contents) === null || _b === void 0
      ? void 0
      : _b.flatMap(c => c.examples || []).map(example => example.key),
  );
...
const MockingButton = ({ operation, options: { code, example, dynamic }, onOptionsChange }) => {
  ...
  const setMockingOptions = React__namespace.useCallback(
  ...
  const menuItems = React__namespace.useMemo(() => {
    ...
    const items =
      (_a =
        operationResponses === null || operationResponses === void 0
          ? void 0
          : operationResponses.filter(operationResponse => Number.isInteger(parseFloat(operationResponse.code)))) ===
        null || _a === void 0
        ? void 0
        : _a.map(generateOperationResponseMenu);
    function generateOperationResponseMenu(operationResponse) {
      ...
      const exampleKeys = lodash.uniq(
        (_a = operationResponse.contents) === null || _a === void 0
          ? void 0
          : _a.flatMap(c => c.examples || []).map(example => example.key),
      );
      const exampleChildren =
        exampleKeys === null || exampleKeys === void 0
          ? void 0
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(mosaic.Menu, {
      ...
      items: menuItems,
      renderTrigger: ({ isOpen }) =>
        React__namespace.createElement(mosaic.FieldButton, { active: isOpen, size: 'sm' }, 'Mock Settings'),
    }),
  );
};
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      ...
      parameters.map(parameter =>
        React__namespace.createElement(ParameterEditor, {
          key: parameter.name,
          parameter: parameter,
...
const useRequestParameters = httpOperation => {
  ...
  const parameterValuesWithDefaults = React__namespace.useMemo(
    () =>
      Object.fromEntries(
        allParameters.map(parameter => {
          ...
          return [
            parameter.name,
            (_a = persistedParameterValues[parameter.name]) !== null && _a !== void 0
              ? _a
              : parameterDefaultValues[parameter.name],
          ];
        }),
      ),
...
function extractAllParameters(httpOperation) {
  ...
  const getRequired = obj => {
    var _a;
    return (_a = obj.required) !== null && _a !== void 0 ? _a : false;
  };
  const pathParameters = lodash.orderBy(
    (_b = (_a = httpOperation.request) === null || _a === void 0 ? void 0 : _a.path) !== null && _b !== void 0
      ? _b
      : [],
    [getRequired, 'name'],
    ['desc', 'asc'],
  ...
  const queryParameters = filterOutAuthorizationParams(
    lodash.orderBy(
      (_d = (_c = httpOperation.request) === null || _c === void 0 ? void 0 : _c.query) !== null && _d !== void 0
        ? _d
        : [],
      [getRequired, 'name'],
      ['desc', 'asc'],
  ...
  const headerParameters = filterOutAuthorizationParams(
    lodash.orderBy(
      (_f = (_e = httpOperation.request) === null || _e === void 0 ? void 0 : _e.headers) !== null && _f !== void 0
        ? _f
        : [],
      [getRequired, 'name'],
      ['desc', 'asc'],
  ...
  return lodash.uniqBy([...pathParameters, ...queryParameters, ...headerParameters], p => p.name);
}
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Panel.Titlebar,
      {
        rightComponent:
          bodyFormat &&
          bodyFormats.length > 1 &&
          React__namespace.createElement(ResponseMenu, {
            types: bodyFormats,
            type: bodyFormat,
    ...
    React__namespace.createElement(
      ...
      React__namespace.createElement(
        ...
        React__namespace.createElement(
          'div',
          { className: `sl-mb-3 sl-text-${getHttpCodeColor(response.status)}` },
          `${response.status} ${(_a = HttpCodeDescriptions[response.status]) !== null && _a !== void 0 ? _a : ''}`,
        ),
        response.bodyText && responseType && ['json', 'xml', 'text'].includes(responseType)
          ? React__namespace.createElement(ResponseCodeViewer, {
              language: 'json',
              value:
          ...
          ? React__namespace.createElement(
              ...
              React__namespace.createElement(mosaic.Image, {
  ...
  React__namespace.createElement(
    ...
    React__namespace.createElement(
      ...
      isNetworkError(error)
        ? React__namespace.createElement(NetworkErrorMessage, null)
        : React__namespace.createElement('p', null, error.message),
    ),
  );
...
const ServersDropdown = ({ servers }) => {
  ...
  return React__namespace.createElement(mosaic.Menu, {
    ...
    renderTrigger: ({ isOpen }) =>
      React__namespace.createElement(
        mosaic.FieldButton,
        { icon: freeSolidSvgIcons.faServer, size: 'sm', active: isOpen },
        (chosenServer === null || chosenServer === void 0 ? void 0 : chosenServer.name) ||
          (chosenServer === null || chosenServer === void 0 ? void 0 : chosenServer.description) ||
          'Server',
      ),
  });
};
...
const TryIt = ({
  ...
  const mediaTypeContent =
    (_c =
      (_b = (_a = httpOperation.request) === null || _a === void 0 ? void 0 : _a.body) === null || _b === void 0
        ? void 0
        : _b.contents) === null || _c === void 0
      ? void 0
      : _c[requestBodyIndex !== null && requestBodyIndex !== void 0 ? requestBodyIndex : 0];
  ...
  const isMockingEnabled =
    mockUrl && (chosenServer === null || chosenServer === void 0 ? void 0 : chosenServer.url) === mockUrl;
  ...
  const getValues = () =>
    Object.keys(bodyParameterValues)
      .filter(param => {
        var _a;
        return (_a = !isAllowedEmptyValues[param]) !== null && _a !== void 0 ? _a : true;
      })
  ...
  const isOnlySendButton =
    !((_d = httpOperation.security) === null || _d === void 0 ? void 0 : _d.length) &&
    !allParameters.length &&
    !formDataState.isFormDataBody &&
    !mediaTypeContent;
  const tryItPanelContents = React__namespace.createElement(
    React__namespace.Fragment,
    null,
    ((_e = httpOperation.security) === null || _e === void 0 ? void 0 : _e.length)
      ? React__namespace.createElement(TryItAuth, {
          onChange: setOperationAuthValue,
          operationSecurityScheme: (_f = httpOperation.security) !== null && _f !== void 0 ? _f : [],
          value: operationAuthValue,
        })
      : null,
    allParameters.length > 0 &&
      React__namespace.createElement(OperationParameters, {
    ...
    formDataState.isFormDataBody
      ? React__namespace.createElement(FormDataBody, {
      ...
      : mediaTypeContent
      ? React__namespace.createElement(RequestBody, {
          examples: (_g = mediaTypeContent.examples) !== null && _g !== void 0 ? _g : [],
    ...
    React__namespace.createElement(
      mosaic.Panel.Content,
      { className: 'SendButtonHolder', mt: 4, pt: !isOnlySendButton && !embeddedInMd ? 0 : undefined },
      React__namespace.createElement(
        mosaic.HStack,
        { alignItems: 'center', spacing: 2 },
        React__namespace.createElement(
          mosaic.Button,
          { appearance: 'primary', loading: loading, disabled: loading, onPress: handleSendRequest, size: 'sm' },
          'Send API Request',
        ),
        servers.length > 1 && React__namespace.createElement(ServersDropdown, { servers: servers }),
        isMockingEnabled &&
          React__namespace.createElement(MockingButton, {
            options: mockingOptions,
            onOptionsChange: setMockingOptions,
            operation: httpOperation,
          }),
      ),
      validateParameters &&
        hasRequiredButEmptyParameters &&
        React__namespace.createElement(
          box.Box,
          null,
  ...
  let tryItPanelElem;
  if (embeddedInMd) {
    tryItPanelElem = React__namespace.createElement(
      mosaic.Panel,
      { isCollapsible: false, p: 0, className: 'TryItPanel' },
      React__namespace.createElement(
        mosaic.Panel.Titlebar,
        { bg: 'canvas-300' },
        React__namespace.createElement(
          box.Box,
          { color: !isDark ? HttpMethodColors[httpOperation.method] : undefined },
          httpOperation.method.toUpperCase(),
        ),
        React__namespace.createElement(
          box.Box,
          { textOverflow: 'truncate', overflowX: 'hidden' },
          `${(chosenServer === null || chosenServer === void 0 ? void 0 : chosenServer.url) || ''}${
            httpOperation.path
          }`,
        ),
      ),
    ...
    tryItPanelElem = React__namespace.createElement(box.Box, { className: 'TryItPanel' }, tryItPanelContents);
  }
  return React__namespace.createElement(
    box.Box,
    { overflowY: 'hidden' },
    ...
    requestData &&
      embeddedInMd &&
      React__namespace.createElement(RequestSamples, { request: requestData, embeddedInMd: true }),
...
const LoadMore = ({ loading, onClick }) => {
  return React__namespace.createElement(
    mosaic.Flex,
    { flexDirection: 'col', justifyContent: 'center', alignItems: 'center', style: { height: '400px' } },
    React__namespace.createElement(
      mosaic.Button,
      { 'aria-label': 'load-example', onPress: onClick, appearance: 'minimal', loading: loading, disabled: loading },
      loading ? 'Loading...' : 'Load examples',
    ),
    React__namespace.createElement(
      mosaic.Text,
      { fontSize: 'base', textAlign: 'center' },
...
const ResponseExamples = ({ httpOperation, responseMediaType, responseStatusCode }) => {
  ...
  const responseContents =
    (_a = response === null || response === void 0 ? void 0 : response.contents) === null || _a === void 0
      ? void 0
      : _a.find(content => content.mediaType === responseMediaType);
  let userDefinedExamples;
  if (
    (responseContents === null || responseContents === void 0 ? void 0 : responseContents.examples) &&
    (responseContents === null || responseContents === void 0 ? void 0 : responseContents.examples.length) > 0
  ) {
  ...
  const examplesSelect =
    userDefinedExamples &&
    userDefinedExamples.length > 1 &&
    React__default['default'].createElement(mosaic.Select, {
  ...
  return React__default['default'].createElement(
    mosaic.Panel,
    { rounded: true, isCollapsible: false },
    React__default['default'].createElement(
      mosaic.Panel.Titlebar,
      {
        rightComponent: React__default['default'].createElement(mosaic.CopyButton, {
          size: 'sm',
          copyValue: responseExample || '',
        }),
      },
      examplesSelect || React__default['default'].createElement(mosaic.Text, { color: 'body' }, 'Response Example'),
    ),
    React__default['default'].createElement(
      mosaic.Panel.Content,
      { p: 0 },
...
const TryItWithRequestSamples = _a => {
  var { hideTryIt } = _a,
    props = tslib.__rest(_a, ['hideTryIt']);
  const [requestData, setRequestData] = React__namespace.useState();
  return React__namespace.createElement(
    mosaic.VStack,
    { spacing: 6 },
...
const TwoColumnLayout = React__default['default'].forwardRef(({ header, right, left, className }, ref) =>
  React__default['default'].createElement(
    mosaic.VStack,
    { ref: ref, w: 'full', className: className, spacing: 8 },
    header,
    React__default['default'].createElement(
      mosaic.Flex,
      null,
      ...
      right &&
        React__default['default'].createElement(
          mosaic.Box,
          { 'data-testid': 'two-column-right', ml: 16, pos: 'relative', w: '2/5', style: { maxWidth: 500 } },
          right,
        ),
    ),
  ),
...
const DeprecatedBadge = () =>
  React__default['default'].createElement(
    mosaic.Tooltip,
    {
      renderTrigger: React__default['default'].createElement(
        mosaic.Badge,
        { intent: 'warning', icon: ['fas', 'exclamation-circle'], 'data-testid': 'badge-deprecated' },
        'Deprecated',
      ),
    },
...
const InternalBadge = ({ isHttpService }) =>
  React__default['default'].createElement(
    mosaic.Tooltip,
    {
      renderTrigger: React__default['default'].createElement(
        mosaic.Badge,
        { icon: freeSolidSvgIcons.faEye, 'data-testid': 'badge-internal', bg: 'danger' },
        'Internal',
      ),
    },
    `This ${isHttpService ? 'operation' : 'model'} is marked as internal and won't be visible in public docs.`,
  );
const VersionBadge = ({ value, backgroundColor }) =>
  React__default['default'].createElement(
    mosaic.Badge,
    {
      appearance: 'solid',
...
function getOAuthFlowDescription(title, flow) {
  ...
  const scopes = lodash.entries(flow.scopes);
  if (scopes.length) {
    description += `\n\nScopes:
...
const SectionTitle = ({ title, id, size = 2, children }) => {
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Box,
      { py: 1, pr: 6, as: LinkHeading, size: size, 'aria-label': title, id: id || slugify(title) },
      title,
    ),
    React__namespace.createElement(
      mosaic.Box,
      { alignSelf: 'center', py: 1, flexGrow: true, style: { minWidth: 0 } },
      children,
    ),
  );
};
...
const SubSectionPanel = ({ title, children, hasContent, rightComponent, defaultIsOpen = true, onChange }) => {
  return React__namespace.createElement(
    mosaic.Panel,
    { isCollapsible: hasContent, defaultIsOpen: defaultIsOpen, onChange: onChange, appearance: 'outlined' },
    React__namespace.createElement(
      mosaic.Panel.Titlebar,
      { fontWeight: 'medium', rightComponent: rightComponent },
...
const Body = ({ body, onChange }) => {
  ...
  const descriptionChanged =
    nodeHasChanged === null || nodeHasChanged === void 0
      ? void 0
      : nodeHasChanged({ nodeId: body.id, attr: 'description' });
  return React__namespace.createElement(
    mosaic.VStack,
    { spacing: 6 },
    React__namespace.createElement(
      SectionSubtitle,
      { title: 'Body', id: 'request-body' },
      contents.length > 0 &&
        React__namespace.createElement(
          mosaic.Flex,
          { flex: 1, justify: 'end' },
    ...
    isJSONSchema(schema) &&
      React__namespace.createElement(jsonSchemaViewer.JsonSchemaViewer, {
...
const Parameters = ({ parameters, parameterType }) => {
  ...
  return React__namespace.createElement(jsonSchemaViewer.JsonSchemaViewer, {
    resolveRef: refResolver,
    schema: schema,
    disableCrumbs: true,
    nodeHasChanged: nodeHasChanged,
...
const httpOperationParamsToSchema = ({ parameters, parameterType }) => {
  ...
  for (const p of sortedParams) {
    ...
    if (json.isPlainObject(schema.properties)) {
      schema.properties[p.name] = Object.assign(Object.assign({}, paramSchema), {
        description: paramDescription,
        examples: [...paramExamples, ...schemaExamplesArray],
        deprecated: paramDeprecated,
        style: paramStyle,
        'x-stoplight': Object.assign(
          Object.assign({}, json.isPlainObject(paramSchema['x-stoplight']) ? paramSchema['x-stoplight'] : {}),
          { id: p.id },
        ),
      });
    }
...
const Request = ({
  ...
  const securitySchemes = lodash.flatten(security);
...
const SecurityPanel = ({ scheme, includeKey }) => {
  ...
  return React__namespace.createElement(
    SubSectionPanel,
    {
      ...
      onChange: isOpen => setExpanded(Object.assign(Object.assign({}, expandedState), { [scheme.key]: isOpen })),
    },
    React__namespace.createElement(MarkdownViewer, {
      style: { fontSize: 12 },
    ...
    schemes.map((scheme, i) =>
      React__namespace.createElement(
        ...
        React__namespace.createElement(SecurityPanel, {
          scheme: scheme,
          includeKey: shouldIncludeKey(schemes, scheme.type),
        }),
        React__namespace.createElement(mosaic.NodeAnnotation, {
          change: nodeHasChanged === null || nodeHasChanged === void 0 ? void 0 : nodeHasChanged({ nodeId: scheme.id }),
        }),
      ),
    ),
  );
...
const Responses = ({ responses: unsortedResponses, onStatusCodeChange, onMediaTypeChange }) => {
  var _a, _b;
  const responses = lodash.sortBy(
    lodash.uniqBy(unsortedResponses, r => r.code),
    r => r.code,
  );
  const [activeResponseId, setActiveResponseId] = React__namespace.useState(
    (_b = (_a = responses[0]) === null || _a === void 0 ? void 0 : _a.code) !== null && _b !== void 0 ? _b : '',
  );
  React__namespace.useEffect(() => {
  ...
  return React__namespace.createElement(
    mosaic.VStack,
    { spacing: 8, as: mosaic.Tabs, selectedId: activeResponseId, onChange: setActiveResponseId, appearance: 'pill' },
    React__namespace.createElement(
      SectionTitle,
      { title: 'Responses' },
      React__namespace.createElement(
        ...
        { density: 'compact' },
        responses.map(({ code }) =>
          React__namespace.createElement(mosaic.Tab, { key: code, id: code, intent: codeToIntentVal(code) }, code),
        ),
      ),
    ),
    React__namespace.createElement(
      ...
      responses.map(response =>
        React__namespace.createElement(
          mosaic.TabPanel,
          { key: response.code, id: response.code },
...
const Response = ({ response, onMediaTypeChange }) => {
  ...
  const descriptionChanged =
    nodeHasChanged === null || nodeHasChanged === void 0
      ? void 0
      : nodeHasChanged({ nodeId: response.id, attr: 'description' });
  return React__namespace.createElement(
    mosaic.VStack,
    { spacing: 8, pt: 8 },
    description &&
...
const HttpOperationComponent = React__namespace.memo(
  ({ className, data: unresolvedData, layoutOptions, tryItCredentialsPolicy, tryItCorsProxy }) => {
    ...
    const header = React__namespace.createElement(OperationHeader, {
      id: data.id,
      method: data.method,
      path: data.path,
      noHeading: layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.noHeading,
      hasBadges: hasBadges,
      name: prettyName,
    ...
    const descriptionChanged =
      nodeHasChanged === null || nodeHasChanged === void 0
        ? void 0
        : nodeHasChanged({ nodeId: data.id, attr: 'description' });
    const description = React__namespace.createElement(
      ...
      data.description &&
        React__namespace.createElement(
          ...
          { pos: 'relative' },
          React__namespace.createElement(MarkdownViewer, {
            className: 'HttpOperation__Description',
      ...
      data.responses &&
        React__namespace.createElement(Responses, {
    ...
    const tryItPanel =
      !(layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.hideTryItPanel) &&
      React__namespace.createElement(TryItWithRequestSamples, {
        httpOperation: data,
        responseMediaType: responseMediaType,
        responseStatusCode: responseStatusCode,
        requestBodyIndex: requestBodyIndex,
        hideTryIt: layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.hideTryIt,
        tryItCredentialsPolicy: tryItCredentialsPolicy,
        mockUrl: mocking.hideMocking ? undefined : mocking.mockUrl,
        corsProxy: tryItCorsProxy,
      });
    return React__namespace.createElement(TwoColumnLayout, {
      className: cn__default['default']('HttpOperation', className),
      header: header,
      left: description,
      right: tryItPanel,
    });
...
function MethodPathInner({ method, path, chosenServerUrl }) {
  ...
  const pathElem = React__namespace.createElement(
    mosaic.Flex,
    { overflowX: 'hidden', fontSize: 'lg', userSelect: 'all' },
    React__namespace.createElement(
      mosaic.Box,
      { dir: 'rtl', color: 'muted', textOverflow: 'truncate', overflowX: 'hidden' },
      leading,
      trailing !== null &&
        React__namespace.createElement(
          mosaic.Box,
          { as: 'span', dir: 'ltr', style: { unicodeBidi: 'bidi-override' } },
          trailing,
        ),
    ),
...
function OperationHeader({ id, noHeading, hasBadges, name, isDeprecated, isInternal, method, path }) {
  ...
  const lineOneChanged =
    nodeHasChanged === null || nodeHasChanged === void 0
      ? void 0
      : nodeHasChanged({ nodeId: id, attr: ['iid', 'summary', 'deprecated', 'internal'] });
  const lineTwoChanged =
    nodeHasChanged === null || nodeHasChanged === void 0
      ? void 0
      : nodeHasChanged({ nodeId: id, attr: ['method', 'path'] });
  return React__namespace.createElement(
    mosaic.VStack,
    { spacing: 5 },
    React__namespace.createElement(
      ...
      React__namespace.createElement(
        ...
        { spacing: 5 },
        !noHeading && name
          ? React__namespace.createElement(mosaic.Heading, { size: 1, fontWeight: 'semibold' }, name)
          : null,
        React__namespace.createElement(
          mosaic.HStack,
          { spacing: 2 },
...
const PoweredByLink = ({ source, pathname, packageType, layout = 'sidebar' }) => {
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      box.Box,
      { alignItems: 'center', marginRight: 'space20' },
      React__namespace.createElement(text.Text, { as: 'p' }, 'Twilio Docs Link'),
    ),
  );
};
...
const AdditionalInfo = ({ id, termsOfService, contact, license }) => {
  ...
  const hasChanged =
    nodeHasChanged === null || nodeHasChanged === void 0
      ? void 0
      : nodeHasChanged({ nodeId: id, attr: ['termsOfService', 'contact', 'license'] });
  const contactLink =
    (contact === null || contact === void 0 ? void 0 : contact.name) &&
    (contact === null || contact === void 0 ? void 0 : contact.url)
  ...
  const licenseLink =
    (license === null || license === void 0 ? void 0 : license.name) && licenseUrl
      ? `[${license.name} License](${licenseUrl})`
      : '';
  const tosLink = termsOfService ? `[Terms of Service](${termsOfService})` : '';
  return contactLink || licenseLink || tosLink
    ? React__default['default'].createElement(
        mosaic.Panel,
        { rounded: true, isCollapsible: false, pos: 'relative' },
        React__default['default'].createElement(
          mosaic.Panel.Titlebar,
          { bg: 'canvas-300' },
        ...
        React__default['default'].createElement(
          ...
          React__default['default'].createElement(
            mosaic.Panel.Content,
            null,
            React__default['default'].createElement(MarkdownViewer, {
              style: { fontSize: 12 },
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(mosaic.Menu, {
      ...
      renderTrigger: ({ isOpen }) =>
        React__namespace.createElement(
          mosaic.Button,
          { iconRight: 'chevron-down', appearance: 'default', ml: 2, active: isOpen, size: 'sm' },
          'Export',
        ),
    }),
  );
...
const SecuritySchemes = ({ schemes, defaultScheme, defaultCollapsed = false }) => {
  return React__default['default'].createElement(
    mosaic.Panel,
    { rounded: true, isCollapsible: defaultCollapsed },
    React__default['default'].createElement(
      mosaic.Panel.Titlebar,
      { bg: 'canvas-300' },
    ...
    React__default['default'].createElement(
      mosaic.Panel.Content,
      { p: 0 },
      lodash.sortBy(schemes, 'type').map((scheme, i) =>
        React__default['default'].createElement(SecurityScheme, {
          key: i,
          scheme: scheme,
...
const SecurityScheme = ({ scheme, defaultIsOpen, isCollapsible, showSchemeKey }) => {
  const { nodeHasChanged } = useOptionsCtx();
  const hasChanged =
    nodeHasChanged === null || nodeHasChanged === void 0 ? void 0 : nodeHasChanged({ nodeId: scheme.id });
  return React__default['default'].createElement(
    mosaic.Panel,
    { defaultIsOpen: defaultIsOpen, isCollapsible: isCollapsible, pos: 'relative' },
    React__default['default'].createElement(
      mosaic.Panel.Titlebar,
      null,
      React__default['default'].createElement(
        mosaic.Box,
        { as: 'span', role: 'heading' },
...
const ServerInfo = ({ servers, mockUrl }) => {
  ...
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Panel,
      { rounded: true, isCollapsible: false, className: 'BaseURLContent', w: 'full' },
      React__namespace.createElement(mosaic.Panel.Titlebar, { whitespace: 'nowrap' }, 'API Base URL'),
      React__namespace.createElement(
        mosaic.Panel.Content,
        { w: 'full', className: 'sl-flex sl-flex-col' },
        React__namespace.createElement(
          mosaic.VStack,
          { spacing: 1, divider: true },
          serversToDisplay.map((server, index) =>
            React__namespace.createElement(ServerUrl, Object.assign({}, server, { key: index })),
          ),
        ),
      ),
    ),
...
const ServerUrl = ({ id, description, url }) => {
  ...
  return React__namespace.createElement(
    mosaic.Box,
    { whitespace: 'nowrap', pos: 'relative' },
    React__namespace.createElement(mosaic.Text, { pr: 2, fontWeight: 'bold' }, description, ':'),
    React__namespace.createElement(
      mosaic.Tooltip,
      {
        placement: 'right',
        renderTrigger: () => React__namespace.createElement(mosaic.Text, { 'aria-label': description }, url),
      },
      !hasCopied &&
        React__namespace.createElement(
          mosaic.Box,
          { p: 1, onClick: onCopy, cursor: 'pointer' },
          'Copy Server URL ',
...
const HttpServiceComponent = React__namespace.memo(
  ({ data: unresolvedData, location = {}, layoutOptions, exportProps }) => {
    ...
    const nameChanged =
      nodeHasChanged === null || nodeHasChanged === void 0 ? void 0 : nodeHasChanged({ nodeId: data.id, attr: 'name' });
    const versionChanged =
      nodeHasChanged === null || nodeHasChanged === void 0
        ? void 0
        : nodeHasChanged({ nodeId: data.id, attr: 'version' });
    const descriptionChanged =
      nodeHasChanged === null || nodeHasChanged === void 0
        ? void 0
        : nodeHasChanged({ nodeId: data.id, attr: 'description' });
    return React__namespace.createElement(
      ...
      { mb: 10 },
      data.name &&
        !(layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.noHeading) &&
        React__namespace.createElement(
          mosaic.Flex,
          ...
          exportProps &&
            !(layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.hideExport) &&
            React__namespace.createElement(ExportButton, Object.assign({}, exportProps)),
        ),
      data.version &&
        React__namespace.createElement(
          mosaic.Box,
      ...
      pathname &&
        (layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.showPoweredByLink) &&
        React__namespace.createElement(PoweredByLink, {
          source: (_a = data.name) !== null && _a !== void 0 ? _a : 'no-title',
          pathname: pathname,
      ...
      React__namespace.createElement(
        mosaic.VStack,
        { spacing: 6 },
        React__namespace.createElement(ServerInfo, {
          servers: (_b = data.servers) !== null && _b !== void 0 ? _b : [],
          mockUrl: mocking.mockUrl,
        }),
        React__namespace.createElement(
          mosaic.Box,
          null,
          ((_c = data.securitySchemes) === null || _c === void 0 ? void 0 : _c.length)
            ? React__namespace.createElement(SecuritySchemes, {
        ...
        React__namespace.createElement(
          mosaic.Box,
          null,
          (((_d = data.contact) === null || _d === void 0 ? void 0 : _d.email) ||
            data.license ||
            data.termsOfService) &&
            React__namespace.createElement(AdditionalInfo, {
              id: data.id,
...
function useIsCompact(layoutOptions) {
  const { ref, breakpoint } = mosaic.useBreakpoints(
    getBreakpoints(layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.compact),
  );
...
const ModelComponent = ({ data: unresolvedData, className, nodeTitle, layoutOptions, exportProps }) => {
  ...
  const nodeId =
    (_a = data === null || data === void 0 ? void 0 : data['x-stoplight']) === null || _a === void 0 ? void 0 : _a.id;
  ...
  const titleChanged =
    nodeHasChanged === null || nodeHasChanged === void 0
      ? void 0
      : nodeHasChanged({ nodeId, attr: ['title', 'internal'] });
  const header =
    (shouldDisplayHeader || isInternal || isDeprecated) &&
    React__namespace.createElement(
      mosaic.Flex,
      { justifyContent: 'between', alignItems: 'center' },
      React__namespace.createElement(
  ...
  const modelExamples =
    !(layoutOptions === null || layoutOptions === void 0 ? void 0 : layoutOptions.hideModelExamples) &&
    React__namespace.createElement(ModelExamples, { data: data, isCollapsible: isCompact });
  const descriptionChanged =
    nodeHasChanged === null || nodeHasChanged === void 0 ? void 0 : nodeHasChanged({ nodeId, attr: 'description' });
  const description = React__namespace.createElement(
    mosaic.VStack,
    { spacing: 10 },
    data.description &&
    ...
    React__namespace.createElement(jsonSchemaViewer.JsonSchemaViewer, {
      ...
      nodeHasChanged: nodeHasChanged,
      skipTopLevelDescription: true,
    }),
  );
  return React__namespace.createElement(TwoColumnLayout, {
    ref: layoutRef,
    className: cn__default['default']('Model', className),
    header: header,
    left: description,
...
const ModelExamples = React__namespace.memo(({ data, isCollapsible = false }) => {
  ...
  return React__namespace.createElement(
    mosaic.Panel,
    { rounded: true, isCollapsible: isCollapsible, defaultIsOpen: !isCollapsible },
    React__namespace.createElement(
      mosaic.Panel.Titlebar,
      {
        rightComponent: selectedExample
          ? React__namespace.createElement(mosaic.CopyButton, { size: 'sm', copyValue: selectedExample })
          : null,
      },
      examplesSelect || React__namespace.createElement(mosaic.Text, { color: 'body', role: 'heading' }, 'Example'),
    ),
    React__namespace.createElement(
      mosaic.Panel.Content,
      { p: 0 },
      show || !exceedsSize(selectedExample)
        ? React__namespace.createElement(mosaicCodeViewer.CodeViewer, {
        ...
        : React__namespace.createElement(LoadMore, { loading: loading, onClick: handleLoadMorePress }),
    ),
  );
});
...
const Docs = React__namespace.memo(_a => {
  var { nodeType, nodeData, useNodeForRefResolving = false, refResolver, nodeHasChanged } = _a,
    commonProps = tslib.__rest(_a, ['nodeType', 'nodeData', 'useNodeForRefResolving', 'refResolver', 'nodeHasChanged']);
  ...
  if (useNodeForRefResolving) {
    elem = React__namespace.createElement(
      InlineRefResolverProvider,
      { document: parsedNode.data, resolver: refResolver },
      elem,
    );
  }
...
const ParsedDocs = _a => {
  var { node } = _a,
    commonProps = tslib.__rest(_a, ['node']);
  switch (node.type) {
    case 'article':
...
const SidebarLayout = React__namespace.forwardRef(
  ({ sidebar, children, maxContentWidth = MAX_CONTENT_WIDTH, sidebarWidth = SIDEBAR_MIN_WIDTH }, ref) => {
    ...
    return React__namespace.createElement(
      mosaic.Flex,
      { ref: ref, className: 'sl-elements-api', pin: true, h: 'full' },
      React__namespace.createElement(
        mosaic.Flex,
        { ref: sidebarRef, onMouseDown: e => e.preventDefault(), style: { maxWidth: `${SIDEBAR_MAX_WIDTH}px` } },
        React__namespace.createElement(
          mosaic.Flex,
          {
            direction: 'col',
      ...
      React__namespace.createElement(
        mosaic.Box,
        { ref: scrollRef, bg: 'canvas', px: 24, flex: 1, w: 'full', overflowY: 'auto' },
        React__namespace.createElement(
          mosaic.Box,
          { style: { maxWidth: `${maxContentWidth - currentSidebarWidth}px` }, py: 16 },
          children,
        ),
      ),
    );
...
const Logo = ({ logo }) => {
  ...
  return React__namespace.createElement(
    mosaic.Box,
    {
      ...
      overflowY: 'hidden',
      overflowX: 'hidden',
      style: { backgroundColor: (_a = logo.backgroundColor) !== null && _a !== void 0 ? _a : 'transparent' },
    },
    logo.href
      ? React__namespace.createElement(
          'a',
          { href: logo.href, target: '_blank', rel: 'noopener noreferrer' },
          React__namespace.createElement('img', { src: logo.url, height: '30px', width: '30px', alt: logo.altText }),
        )
      : React__namespace.createElement('img', { src: logo.url, height: '30px', width: '30px', alt: logo.altText }),
  );
};
...
function isPartialHttpRequest(maybeHttpRequest) {
  return (
    lodash.isObject(maybeHttpRequest) &&
    ...
    title &&
      React__default['default'].createElement(
        ...
        React__default['default'].createElement(mosaic.Icon, {
          icon: NodeTypeIconDefs[types.NodeType.Model],
          color: NodeTypeColors[types.NodeType.Model],
        }),
        React__default['default'].createElement(mosaic.Box, { color: 'muted', px: 2 }, title),
      ),
    React__default['default'].createElement(jsonSchemaViewer.JsonSchemaViewer, {
      resolveRef: resolveRef,
      schema: getOriginalObject(schema),
    }),
  );
...
const CodeComponent = props => {
  ...
  if (http) {
    ...
    return React__default['default'].createElement(
      PersistenceContextProvider,
      null,
      React__default['default'].createElement(TryIt, {
    ...
    request: Object.assign(
      {
        ...
        path:
          pathParam === null || pathParam === void 0
            ? void 0
            : pathParam.map(name => ({
...
const MarkdownComponentsProvider = ({ value, children }) => {
  return React__namespace.createElement(
    markdownViewer.MarkdownViewerProvider,
    { components: Object.assign({ code: CodeComponent }, value) },
    children,
  );
};
...
const ReactRouterMarkdownLink = ({ title, to, href: _href, children }) => {
  ...
  if (isExternal) {
    return React__default['default'].createElement(
      'a',
      { target: '_blank', rel: 'noreferrer noopener', href: href, title: title },
      children,
    );
  }
...
const TableOfContents = React__namespace.memo(
  ({ tree, activeId, Link, maxDepthOpenByDefault, externalScrollbar = false, onLinkClick }) => {
    ...
    return React__namespace.createElement(
      mosaic.Box,
      { ref: container, w: 'full', bg: 'canvas-100', overflowY: 'auto' },
      React__namespace.createElement(
        mosaic.Box,
        { ref: child, my: 3 },
        React__namespace.createElement(
          ...
          React__namespace.createElement(
            ...
            tree.map((item, key) => {
              ...
              return React__namespace.createElement(GroupItem, {
                key: key,
                item: item,
                depth: 0,
...
const GroupItem = React__namespace.memo(({ item, depth, maxDepthOpenByDefault, onLinkClick }) => {
  if (isExternalLink(item)) {
    return React__namespace.createElement(
      mosaic.Box,
      { as: 'a', href: item.url, target: '_blank', rel: 'noopener noreferrer', display: 'block' },
      React__namespace.createElement(Item, {
        depth: depth,
        title: item.title,
        meta: React__namespace.createElement(mosaic.Box, { as: mosaic.Icon, icon: ['fas', 'external-link'] }),
      }),
    );
  } else if (isGroup(item) || isNodeGroup(item)) {
    return React__namespace.createElement(Group, {
      depth: depth,
      item: item,
      maxDepthOpenByDefault: maxDepthOpenByDefault,
      onLinkClick: onLinkClick,
    ...
    return React__namespace.createElement(Node, {
      depth: depth,
      item: item,
      onLinkClick: onLinkClick,
      meta: item.meta
        ? React__namespace.createElement(
            mosaic.Box,
            { color: NODE_META_COLOR[item.meta], textTransform: 'uppercase', fontWeight: 'medium' },
            item.meta,
          )
        : NODE_TYPE_META_ICON[item.type] &&
          React__namespace.createElement(
            ...
            React__namespace.createElement(mosaic.Box, {
              as: mosaic.Icon,
              color: NODE_TYPE_ICON_COLOR[item.type],
...
const Group = React__namespace.memo(({ depth, item, maxDepthOpenByDefault, onLinkClick = () => {} }) => {
  ...
  const meta = React__namespace.createElement(
    mosaic.Flex,
    { alignItems: 'center' },
  ...
  let elem;
  if (isNodeGroup(item)) {
    elem = React__namespace.createElement(Node, {
      depth: depth,
      item: item,
      meta: meta,
    ...
    elem = React__namespace.createElement(Item, {
      title: item.title,
      meta: meta,
      onClick: handleClick,
      depth: depth,
...
const Item = React__namespace.memo(({ depth, isActive, id, title, meta, icon, onClick }) => {
  return React__namespace.createElement(
    ...
    React__namespace.createElement(
      mosaic.Box,
      { alignItems: 'center', flex: 1, mr: meta ? 1.5 : undefined, ml: icon && 1.5, textOverflow: 'truncate' },
      title,
    ),
...
const NonIdealState = ({ description, icon, title }) => {
  return React__namespace.createElement(
    mosaic.Flex,
    { flexDirection: 'col', alignItems: 'center', justifyContent: 'center', textAlign: 'center', w: 'full', h: 'full' },
    React__namespace.createElement(mosaic.Box, {
      as: mosaic.Icon,
...
const useRouter = (router, basePath, staticRouterPath) => {
  ...
  const routerProps = Object.assign(
    Object.assign({}, router !== 'memory' && { basename: basePath }),
    ...
    doBundle(document, baseUrl)
      ...
      .catch(reason => {
        ...
        if (typeof reason === 'object' && reason !== null && 'files' in reason) {
          ...
          console.warn(
            `Could bundle: ${
              (_a = reason === null || reason === void 0 ? void 0 : reason.message) !== null && _a !== void 0
                ? _a
                : 'Unknown error'
            }`,
          );
...
class Styled extends React__namespace.Component {
  ...
  render() {
    return React__namespace.createElement(
      mosaic.Box,
      { className: 'sl-elements sl-antialiased', fontFamily: 'ui', fontSize: 'base', color: 'body', h: 'full' },
      this.props.children,
    );
  }
}
  ...
  return class extends HTMLElement {
    constructor() {
      ...
      Object.defineProperties(
        this,
        lodash.mapValues(propDescriptors, (_, key) => ({
          get: () => {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      if (propDescriptors[name]) {
        ...
        if (!lodash.isEqual(this._props[name], newPropValue)) {
    ...
    _renderComponent() {
      if (this._mountPoint) {
        const props = lodash.mapValues(propDescriptors, (descriptor, key) => {
          var _a;
          return (_a = this._props[key]) !== null && _a !== void 0 ? _a : descriptor.defaultValue;
        });
